...
```console
nextflow run nf-core/aquascope --input samplesheet.csv --genome GRCh37 -profile docker
```
...
First, go to the [nf-core/aquascope releases page](https://github.com/nf-core/aquascope/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
